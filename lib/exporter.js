...
function TableExporter ($, inColDelim, inRowDelim)  {
    ...
    this.export = function ($table, tableIndex, selector, callback) {
        ...
        switch (selectors.length) {
            case 4:
                targetSelector = selectors[3];
            case 4:
                cellSelector = selectors[2];
            case 3:
        ...
        findHeaderSelector = (headerSelector || 'tr') + ':has(' + (headerSelector || 'th') + ')'; // 'tr:has(th)'
        var $headers = null;
        ...
        var headersMap = $headers.map(toColumns);
